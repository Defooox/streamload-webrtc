...
uploadBtn.onclick = async () => {
    ...
    if (!file) {
        setStatus('uploadStatus', 'error', '⚠️ Выберите файл!');
        return;
    }
...
stopStreamBtn.onclick = () => {
    ...
    setStatus('syncStatus', 'info', '⏸️ Стрим остановлен');
};
...
function syncVideo(sync) {
    ...
    // Синхронизируем время, если разница > 500ms
    if (timeDiff > 0.5) {
...
// Отправка позиции воспроизведения (для будущего расширения)
setInterval(() => {
    if (dataChannel && dataChannel.readyState === 'open') {
        // Можно отправлять команды от клиента
        // dataChannel.send(JSON.stringify({ action: 'pause' }));
    }
}, 100);