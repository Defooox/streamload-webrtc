...
void SharedState::send(std::string message, std::shared_ptr<WebSocketSession> sender) {
    try {
        ...
        if (it == session_to_client_id_.end()) {
            std::cerr << "⚠️ Session not found in client ID map" << std::endl;
            return;
        }
        ...
        if (type == "start_stream") {
            ...
            std::cout << "🚀 Start stream command from " << client_id << ": " << file_path << std::endl;
        ...
        // === СТОП СТРИМА ===
        if (type == "stop_stream") {
            std::cout << "⏹️ Stop stream command from " << client_id << std::endl;
        ...
        if (type == "offer") {
            ...
            // Пробрасываем другие сообщения всем сессиям
        ...
        std::cerr << "⚠️ JSON parse error: " << e.what() << std::endl;
    }
}
